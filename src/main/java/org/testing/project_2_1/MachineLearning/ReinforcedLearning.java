...
public class ReinforcedLearning {
    ...
    private GameState findSuccessorGameState(GameState gameState) {
        GameState successorGameState = new GameState(gameState); // Clone the current state.
        for (Move move : currentTurn.getMoves()) {
            successorGameState.move(move); // Apply the moves of the current turn.
        }
        currentTurn = turns.remove(0); // Move to the next turn.
        return successorGameState; // Return the successor state.
    }
    ...
    private void updateWeights(GameState gameState, double learningRate, double targetValue) {
        double predictedValue = gameState.evaluateBoard(weights); // Predicted value.
        double error = targetValue - predictedValue; // Calculate error.

        // Update each weight using the error and corresponding feature value.
